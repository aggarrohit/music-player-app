...
export const useMusicPlayer = (): UseMusicPlayerReturn => {
  ...
  const markChallengeComplete = useMusicStore(
    (state) => state.markChallengeComplete
  );
  ...
  useEffect(() => {
    if (
      ...
      return; // 🚨 Prevents triggering a false state update when modal opens
    }
  ...
  useEffect(() => {
    if (currentTrack && progress.position > 0) {
      ...
      if (progressPercentage == 100 && !currentTrack.completed) {
